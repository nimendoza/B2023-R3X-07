...
class AllocateAgent:
  ...
  def __init__(self, data: Data):
    ...
    self.shifts = list(data.shifts)
    self.courses = list(data.courses.values())
    self.finyear_students = set(data.students[
      data.grade_levels['Grade 12']
    ])
      ...
      for section in filter(
        ...
        for student in list(section.students):
          for type, _ in filter(
            ...
            if section.parallel_session.session:
            ...
            for type, student in pairs:
              ...
              if (
                ...
                while core == elec or core in elec.not_alongside:
                  student.rankings.final.pop(
                    self.course_types[CORE], 'Cannot be taken with CSE', 0)
                  core = student.rankings.current(self.course_types[CORE])
                  countdown -= 1
                  if not countdown:
                    assert False
